...
@ISA = qw(Wx::Frame);
use base 'Wx::Frame';
@Timing::DataCoding::DataCoderAppCon::ISA = qw(Timing::DataCoding);
...
#use Timing::DataCoding::AppCon::TSWxPoeLayoutDataCoder;
BEGIN { 
	eval { require Timing::DataCoding::AppCon::TSWxPoeLayoutDataCoder }
		or die "Error: Timing::DataCoding::AppCon::TSWxPoeLayoutDataCoder not installed: $@";
   import Timing::DataCoding::AppCon::TSWxPoeLayoutDataCoder;
}
...
my $_wxframe_title = 'DataCoder';
my $_wxframe_ident = 'mainframe';
my $_wxframe_name_key = 'main';
my $_layout_class = 'TSWxPoeLayoutDataCoder';
...
sub send_notice_to_forward_data { 
	...
	my $sig = 'push_go';
	my $val = 1;
	print "[wxframe - SEND NOTICE - FWD DATA] setting signal; sigkey[$sig] sigval[$val]\n" if $carp;
	if($_wfmgr) {
		$try = $_wfmgr->signal_an_event(carp => $carp, sigkey => 'push_go', sigvalue => 1);
	}
	if(!$try) {
		$winframe->push_new_signal( $sig, $val );
	}
	return 1;
}
